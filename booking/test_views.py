...
class TestBookingViews(TestCase):
    ...
    def test_create_booking_view_post(self):
        ...
        self.assertTrue(
            Reservation.objects.filter(comment="Test reservation").exists(),
            msg="Reservation should be created"
        )

    def test_delete_booking_view_post(self):
        """Test deleting an existing reservation"""
        response = self.client.post(
            reverse("delete_booking", args=[self.reservation.pk])
        )  # Send DELETE request for reservation
        self.assertEqual(
            response.status_code, 302,
            msg="Response should redirect after deletion"
        )
        self.assertFalse(
            Reservation.objects.filter(id=self.reservation.id).exists(),
            msg="Reservation should be deleted"
        )
    ...
    def test_edit_booking_view_post(self):
        ...
        response = self.client.post(
            reverse("edit_booking", args=[self.reservation.pk]),
            {
        ...
        self.assertEqual(
            self.reservation.comment, "Updated reservation",
            msg="The reservation comment should be updated"
        )

        # Assert that the phone number was updated
        self.assertEqual(
            self.reservation.phone_number, "+12345678912",
            msg="The phone number should be updated"
        )

        # Assert that the date was updated
        self.assertEqual(
            self.reservation.date, date(2025, 2, 22),
            msg="The date should be updated"
        )

        # Assert that the time was updated
        self.assertEqual(
            self.reservation.time, time(18, 40),
            msg="The time should be updated"
        )

        # Assert that the number of people was updated
        self.assertEqual(
            self.reservation.people_count, 5,
            msg="The number of people should be updated"
        )

        # Assert that the table count was updated
        self.assertEqual(
            self.reservation.table_count, 2,
            msg="The table count should be updated"
        )
